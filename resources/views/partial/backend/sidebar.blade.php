@php
    ...
    // dd(getParentShowOf($current_page),getParentOf($current_page));
@endphp
                ...
                @else
                    <li
                        ...
                        @if ($menu->appearedChildren && count($menu->appearedChildren) > 0 !== null)
                            <div id="collapse_{{ $menu->route }}"
                                class="{{ in_array($menu->parent_show, [getParentShowOf($current_page), getParentOf($current_page)]) ? 'show' : null }}"
                                aria-labelledby="heading_{{ $menu->route }}" data-parent="#accordionSidebar">
                                <div class="bg-white py-2 collapse-inner rounded">
                                    @foreach ($menu->appearedChildren as $sub_menu)
                                        @permission($sub_menu->name)
                                            <a class="collapse-item {{ getParentOf($current_page) != null && (int) (getParentIdOf($current_page) + 1) == $sub_menu->id ? 'active' : null }}"
                                                href="{{ route('admin.' . $sub_menu->as) }}">Buttons</a>
                                        @endpermission
                                    @endforeach
                                </div>
                            </div>
